...
## Repository Contents
- `Detecting_Potential_Opioid_Overprescribing.ipynb`  
  Main notebook with:
  - data cleaning & standardization
  - ICD enrichment & cohort filtering
  - claim-to-PDE linkage logic
  - provider metrics + outlier views

(Optional recommended additions)
- `data/` (ignored via `.gitignore`)
- `docs/` for screenshots or methodology notes
- `sql/` for Athena query equivalents (if you implement)

---

## How to Run
1. Open the notebook in Google Colab (or a Spark-enabled environment)
2. Ensure your inputs are available (e.g., from Google Drive mount or S3 reads):
   - claims dataset
   - PDE dataset
   - ICD reference
   - opioid NDC list
3. Run cells in order:
   - **standardize → enrich → filter pain claims → join to PDE → metrics**

---

## Key Assumptions / Caveats
- A time-window linkage **does not prove causality**—it’s a proxy for temporal association.
- Claims and PDE are linked at the beneficiary level; prescriber in PDE (`PRSCRBR_ID`) may differ from claim’s provider (`PRF_PHYSN_NPI`).
- Small provider volumes can inflate rates; volume thresholds are important for interpretation.
- Keyword-based diagnosis filtering is intentionally simple for interpretability; it can be expanded to a curated ICD list.

---

## Next Enhancements (Optional)
If you want to strengthen the analysis without changing the core concept:
- Add more robust pain ICD code lists (vs keyword matching)
- Compare multiple linkage windows (0–3 vs 0–7 vs 0–14)
- Add provider peer grouping (specialty, geography) if available
- Export curated tables to Parquet and write Athena queries / dashboards

---

## Summary
This project demonstrates an end-to-end healthcare analytics workflow that:
- Standardizes CMS-style claims + PDE data
- Links diagnosis claims to prescription fills via an interpretable time-window rule
- Flags opioid utilization via NDC reference
- Produces provider-level metrics and outlier-oriented views to highlight potential abnormal prescribing behavior
